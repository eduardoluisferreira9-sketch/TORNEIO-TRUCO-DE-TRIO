...
def disparar_atualizacao_placar(m_str, j1, j2):
    ...
    # 🎯 GRAVAÇÃO DINÂMICA DO ESTADO DE CONCLUSÃO REAL (True se algum trio fez 2 sets)
    status_concluido = (s1 == 2 or s2 == 2)
    st.session_state["placares_rodada_atual"][m_str] = [s1, s2, t1, t2, f1, f2, status_concluido]
    salvar_estado_no_disco()
...
def desenhar_mesa_planta_baixa(j1, j2, mesa_num, s1, t1, f1, s2, t2, f2, status_concluido=False, tipo_jogo="normal"):
    ...
    if tipo_jogo == "final":
        ...
        card_height = "370px"
        fonte_jogadores = "1.1rem"
        animacao_css = "animation: pulsarAndamento 3s infinite ease-in-out;"
    ...
    @keyframes pulsarAndamento {{
        0% {{ box-shadow: 0px 0px 8px rgba(230,126,34,0.3); }}
        50% {{ box-shadow: 0px 0px 20px rgba(230,126,34,0.7); }}
        100% {{ box-shadow: 0px 0px 8px rgba(230,126,34,0.3); }}
    }}
    </style>
...
with st.sidebar:
    ...
    if is_admin:
        ...
        if st.button("🗑️ LIMPAR GALERIA HISTÓRICA"):
            if os.path.exists(ARQUIVO_GALERIA): os.remove(ARQUIVO_GALERIA)
            st.sidebar.success("Galeria Histórica Apagada!")
            st.rerun()
        ...
        if not st.session_state["em_matamata"]:
            ...
            for j1, j2 in st.session_state["confrontos"]:
                if j2 == "CHAPÉU (Folga)":
                    ...
                    with col_alvo:
                        desenhar_mesa_planta_baixa(j1, j2, str(cont_m), p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo="normal")
                    cont_m += 1
        else:
            if st.session_state["fase_matamata"] == "FINAL E TERCEIRO":
                ...
                for c in st.session_state["confrontos_mm"]:
                    ...
                    if c["tipo"] == "final":
                        with col_f1:
                            desenhar_mesa_planta_baixa(c["j1"], c["j2"], c["id_original"], p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo="final")
                    elif c["tipo"] == "3place":
                        with col_f2:
                            desenhar_mesa_planta_baixa(c["j1"], c["j2"], c["id_original"], p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo="3place")
            else:
                ...
                for c in st.session_state["confrontos_mm"]:
                    ...
                    p = st.session_state["placares_rodada_atual"].get(c["id_original"], [0,0,0,0,0,0,False])
                    with col_alvo:
                        desenhar_mesa_planta_baixa(c["j1"], c["j2"], c["id_original"], p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo="normal")
    else:
                ...
                if not st.session_state["em_matamata"]:
                    ...
                    for j1, j2 in st.session_state["confrontos"]:
                        if j2 != "CHAPÉU (Folga)":
                            ...
                            if is_admin:
                                ...
                                with col_painel: desenhar_mesa_planta_baixa(j1, j2, m, p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo="normal")
                                with col_entradas: renderizar_formulario_mesa_admin(m, j1, j2, sem_id)
                            else: desenhar_mesa_planta_baixa(j1, j2, m, p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo="normal")
                            cont += 1
                        ...
                        if st.button("Validar e Encerrar Rodada Atual", type="primary"):
                            ...
                            for j1, j2 in st.session_state["confrontos"]:
                                if j2 != "CHAPÉU (Folga)":
                                    ...
                                    if not p[6]: st.error(f"❌ Mesa {m_c}: Partida incompleta!"); erro_v = True
                                    m_c += 1
                            if not erro_v:
                    ...
                    for c in st.session_state["confrontos_mm"]:
                        ...
                        if is_admin:
                            ...
                            with col_p_mm: desenhar_mesa_planta_baixa(j1, j2, m, p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo=c["tipo"])
                            with col_e_mm: renderizar_formulario_mesa_admin(m, j1, j2, sem_id)
                        else: desenhar_mesa_planta_baixa(j1, j2, m, p[0], p[2], p[4], p[1], p[3], p[5], status_concluido=p[6], tipo_jogo=c["tipo"])
                    ...
                    if is_admin:
                        if st.button("🏆 Validar e Avançar Playoffs", type="primary"):
                            ...
                            for c in st.session_state["confrontos_mm"]:
                                p = st.session_state["placares_rodada_atual"].get(c["id_original"], [0,0,0,0,0,0,False])
                                if not p[6]: st.error(f"❌ Partida pendente na mesa {c['id_original']}!"); erro_mm = True
                            if not erro_mm:
                                venc, perd = [], []
                                for c in st.session_state["confrontos_mm"]:
                                    p = st.session_state["placares_rodada_atual"].get(c["id_original"], [0,0,0,0,0,0,False])
...
st.markdown("""
    ...
    ">
        ...
        <div style="color: #ffffff; font-size: 0.85rem; font-weight: bold; display: flex; gap: 15px; align-items: center; text-shadow: 1px 1px 2px #000;">
            <span>📦 Versão: <span style="color: #ffb703; font-weight: 900;">2.8.5-Testing</span></span>
