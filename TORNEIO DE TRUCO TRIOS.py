...
st.set_page_config(
    page_title="Central de Torneios de Truco MTG - Trios",
    page_icon="🃏",
    layout="wide"
)
...
def reconstruir_classificacao_global():
    ...
    entidades_trios = [j["entidade"] for j in st.session_state["jogadores"]]
    
    st.session_state["classificacao"] = pd.DataFrame({
        'Trio': nomes_trios, 'Entidade': entidades_trios, 'Vitorias': 0, 'Amarrados_Ganhos': 0, 
    ...
    for r_num, mesas in st.session_state["historico_rodadas"].items():
        for m_id, dados in mesas.items():
            if dados.get("is_chapeu", False):
                ...
                st.session_state["classificacao"].loc[j1, ['Vitorias','Amarrados_Ganhos','Tentos_Pro','Tentos_Contra','Flores']] += [v1, s1_c, t1, t2, f1]
                st.session_state["classificacao"].loc[j2, ['Vitorias','Amarrados_Ganhos','Tentos_Pro','Tentos_Contra','Flores']] += [v2, s2_c, t2, t1, f2]
...
def disparar_atualizacao_placar(m_str, j1, j2):
    ...
    if (s1 == 2 and s2 == 0):
        ...
        t2, t1 = 72, min(st.session_state.get(f"dir_t1_{m_str}_r{sem}_2x0j2", p_antigo[2]), 48)
    else:
...
# --- CARDS DINÂMICOS DE MESA (PLANTA BAIXA PREMIUM) ---
def desenhar_mesa_planta_baixa(j1, j2, mesa_num, s1, t1, f1, s2, t2, f2, tipo_jogo="normal"):
    ent1 = obter_entidade_do_trio(j1)
    ent2 = obter_entidade_do_trio(j2)
    animacao_css = ""
    ...
    <div style="background: linear-gradient(135deg, #0f2d1b, #06170d); border: 4px solid {borda_cor}; border-radius: 20px; padding: 12px; display: flex; flex-direction: column; align-items: center; justify-content: space-between; position: relative; box-shadow: 0px 8px 16px rgba(0,0,0,0.5); height: {card_height}; box-sizing: border-box; color: #ffffff; font-family: system-ui, -apple-system, sans-serif; margin-bottom: 5px; {animacao_css}">
        
        <div style="text-align: center; width: 100%;">
            <div style="font-size: 0.75rem; color: #69db7c; font-weight: bold; text-transform: uppercase;">🧔 Trio 1 ({ent1})</div>
        ...
        <div style="background-color: #04120a; border: 2px solid {borda_cor}; border-radius: 12px; padding: 10px; width: 95%; text-align: center; box-shadow: inset 0 2px 5px rgba(0,0,0,0.6);">
            ...
            <div style="display: flex; justify-content: space-around; align-items: center; font-size: 2.2rem; font-weight: 900; margin-top: 8px;">
                <div style="color: #ffb703;">{int(s1)}<span style="font-size:1.2rem; color:#69db7c;">am</span> {int(t1)}<span style="font-size:1.2rem; color:#69db7c;">t</span></div>
                <div style="font-size: 1rem; color: #69db7c; font-weight: bold;">X</div>
                <div style="color: #ffffff;">{int(s2)}<span style="font-size:1.2rem; color:#69db7c;">am</span> {int(t2)}<span style="font-size:1.2rem; color:#69db7c;">t</span></div>
            </div>
        ...
        <div style="text-align: center; width: 100%;">
            ...
            <div style="font-size: 0.75rem; color: #69db7c; font-weight: bold; text-transform: uppercase; margin-top: 2px;">🧔 Trio 2 ({ent2})</div>
        </div>
    </div>
    """
...
def renderizar_formulario_mesa_admin(m, j1, j2, sem_id):
    ...
    with c1:
        st.markdown(f"<h4 class='titulo-passo-admin'>• AMARRADOS (Passo 1)</h4>", unsafe_allow_html=True)
        s1_in = st.number_input(f"Amarrados - {j1}", 0, 2, int(s1), key=f"dir_s1_{m}_r{sem_id}", on_change=disparar_atualizacao_placar, args=(m, j1, j2))
        s2_in = st.number_input(f"Amarrados - {j2}", 0, 2, int(s2), key=f"dir_s2_{m}_r{sem_id}", on_change=disparar_atualizacao_placar, args=(m, j1, j2))

    jogo_encerrado = (s1_in == 2 or s2_in == 2)
    
    with c2:
        if not jogo_encerrado:
            st.warning("Aguardando Amarrados...")
        else:
            st.markdown(f"<h4 class='titulo-passo-admin'>• TENTOS (Passo 2)</h4>", unsafe_allow_html=True)
            if s1_in == 2 and s2_in == 0:
                st.number_input(f"Tentos - {j2} (Máx: 48)", 0, 48, min(int(t2), 48), key=f"dir_t2_{m}_r{sem_id}_2x0j1", on_change=disparar_atualizacao_placar, args=(m, j1, j2))
            elif s2_in == 2 and s1_in == 0:
                st.number_input(f"Tentos - {j1} (Máx: 48)", 0, 48, min(int(t1), 48), key=f"dir_t1_{m}_r{sem_id}_2x0j2", on_change=disparar_atualizacao_placar, args=(m, j1, j2))
            else:
...
if modo_exibicao == "🖥️ MODO TELÃO DE PROJETOR (Automático)":
    ...
    if st.session_state["torneio_iniciado"]:
        if not st.session_state["em_matamata"]:
            ...
            for j1, j2 in st.session_state["confrontos"]:
                if j2 == "CHAPÉU (Folga)":
                    st.markdown(f'<div class="chapeu-container-novo"><h3 style="color:#ffb703; margin:0;">🎩 FOLGA REGULAMENTAR (CHAPÉU): {j1} ({obter_entidade_do_trio(j1)})</h3></div>', unsafe_allow_html=True)
                else:
        ...
        st.info("Aguardando o início do torneio para projetar as chaves.")
    ...
    with aba_arena:
        if not st.session_state["torneio_iniciado"]:
            st.markdown("### 🎮 Inscrições de Trios (Normas MTG)")
            ...
            if is_admin:
                if st.session_state.get("jogador_sendo_editado") is not None:
                    ...
                    with st.form("form_edicao"):
                        novo_nome = st.text_input("Corrigir Nome do Trio:", value=dados_antigos["trio"])
                        nova_entidade = st.text_input("Corrigir Entidade:", value=dados_antigos["entidade"])
                        col_b1, col_b2 = st.columns(2)
                        with col_b1:
                            if st.form_submit_button("💾 Salvar") and novo_nome.strip() and nova_entidade.strip():
                                st.session_state["jogadores"][idx_edit] = {"trio": novo_nome.strip(), "entidade": nova_entidade.strip()}
                    ...
                    with st.form("cad", clear_on_submit=True):
                        ...
                        with c_in1: nj = st.text_input("Nome do Trio / Identificação:")
                        with c_in2: ne = st.text_input("Entidade Tradicionalista (Ex: CTG Lalá):")
                        if st.form_submit_button("➕ Cadastrar Equipe") and nj and ne:
                            st.session_state["jogadores"].append({"trio": nj.strip(), "entidade": ne.strip()})
                            salvar_estado_no_disco(); st.rerun()
                            
            st.write(f"**Trios Registrados ({len(st.session_state['jogadores'])}):**")
            if st.session_state["jogadores"]:
                if is_admin:
                    for idx, j_dados in enumerate(st.session_state["jogadores"]):
                        c_nome, c_edit, c_excluir = st.columns([70, 15, 15])
                        with c_nome: st.markdown(f"<p style='padding:8px; background-color:#0d301b; border-radius:6px; font-weight:bold; border: 1px solid #ffb703;'>🔹 <b>{j_dados['trio']}</b> <span style='color:#ffb703;'>[{j_dados['entidade']}]</span></p>", unsafe_allow_html=True)
                        with c_edit:
                    ...
                    st.info(", ".join([f"{j['trio']} ({j['entidade']})" for j in st.session_state["jogadores"]]))
            ...
            with c_m1: st.markdown(f'<div class="metric-panel"><div class="metric-val">{len(st.session_state["jogadores"])}</div><div class="metric-lbl">Trios Ativos</div></div>', unsafe_allow_html=True)
            with c_m2:
                ...
                st.markdown(f'<div class="metric-panel"><div class="metric-val">{fase_txt}</div><div class="metric-lbl">Estágio do Campeonato</div></div>', unsafe_allow_html=True)
            with c_m3:
            ...
            if st.session_state["campeao"]:
                ...
                if not st.session_state["em_matamata"]:
                    ...
                    for j1, j2 in st.session_state["confrontos"]:
                        if j2 == "CHAPÉU (Folga)":
                            st.markdown(f'<div class="chapeu-container-novo"><div class="metric-lbl">🎩 Trio no Chapéu (Folga)</div><div class="cronometro-tempo" style="font-size:2rem!important;">{j1} ({obter_entidade_do_trio(j1)})</div></div>', unsafe_allow_html=True)
    ...
    with aba_tabela:
        if st.session_state["classificacao"] is not None:
            ...
            df_r = st.session_state["classificacao"].sort_values(by=['Vitorias','Amarrados_Ganhos','Saldo_Tentos'], ascending=False)
            st.table(df_r)
                    ...
                    for m_id, dados in st.session_state["historico_rodadas"][r_sel].items():
                        if not dados.get("is_chapeu", False):
                            ...
                            if is_admin:
                                ...
                                with c_e1:
                                    st.number_input(f"Amarrados ({dados['j1']})", 0, 2, int(dados["s1"]), key=f"ret_s1_{r_sel}_{m_id}", on_change=salvar_mudanca_retroativa, args=(r_sel, m_id, dados['j1'], dados['j2']))
                                ...
                                with c_e2:
                                    st.number_input(f"Amarrados ({dados['j2']})", 0, 2, int(dados["s2"]), key=f"ret_s2_{r_sel}_{m_id}", on_change=salvar_mudanca_retroativa, args=(r_sel, m_id, dados['j1'], dados['j2']))
                            ...
                            else: st.markdown(f"👉 **Placar Histórico:** {dados['s1']}am {dados['t1']}t VS {dados['s2']}am {dados['t2']}t")
...
# --- RODAPÉ INSTITUCIONAL DE ULTRA CONTRASTE ---
st.markdown("""
